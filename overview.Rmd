...
Prior to starting this project I hadn't seen any other examples of this data being analyzed in this way, though of course in the process of doing further research for it, I did eventually find other takes on this data (specifically, this web report from the Institut der deutschen Wirtschaft: https://www.iwkoeln.de/studien/iw-kurzberichte/beitrag/wido-geis-anja-katrin-orth-weniger-frauen-gehen-maennerueberschuss-bleibt-bestehen-302444.html). Their map will look differently from mine, because they chose to focus on a somewhat broader age range (20 - 49-year-olds) than I did.
...
map_allages <- ggplot(data = landkreisen_fortified_geschlecht,
       aes(long, lat, fill = difference, group = group, text = paste("<b>", Kreise, "</b>", "<br>","M-F Difference (%):", difference))) +
...
style(map_allages_ly, hoverlabel = list(bgcolor = "white"), hoveron = "fills") # "fills"" ties the hover text to the shape of the Landkreis -- stops it from grabbing data from neighboring Kreisen
...
style(map_1835_ly, hoverlabel = list(bgcolor = "white"), hoveron = "fills") 
...
# map_1835_breaks_ly
style(map_1835_breaks_ly, hoverinfo = "text", hoverlabel = list(bgcolor = "white"), hoveron = "fills") 
# breaks on hoveron = "fills" -- trying to show too much info?
```
...
* The next four leading Landkreisen -- **Ilm**, **Aachen**, **Darmstadt**, and **Karlsruhe** -- are also well-known for their technical universities. Chance are, students in these cities will not be the least bit surprised if you tell them they are living in some of the most male-skewing areas in Germany: they already know.
...
4) Can we observe migrational patterns with time series data?